...
class MapItem
{
	...
	public static MapItem unmarshal(Json ob) {
		...
		return newItem((int)ob.getLong("x"), (int)ob.getLong("y"), (int)ob.getLong("type"));
	}
	Point pos(int time)
	{
	...
	public static MapItem newItem(int x, int y, int type) {
		if (type == 3 || type == 9)
			...
			return new Horizontal(x, y, type);
		else if (type == 2)
			return new Scaler(x, y, type);
		else
			return new MapItem(x, y, type);
	}
	public double scale(int time)
	{
		return 1.0;
	}
...
class Jumper extends MapItem
{
	...
	Jumper(int x, int y, int type)
	{
		...
		this.y = y;
		this.type = type;
		//this.super(this.x, this.y, this.type);
...
class Horizontal extends MapItem
{
	int x;
	int y;
	int type;
	...
	Horizontal(int x, int y, int type)
	{
		super(x, y, type);
		this.x = x;
		this.y = y;
...
class Scaler extends MapItem
{
	int x;
	int y;
	int type;

	Scaler(int x, int y, int type)
	{
		super(x, y, type);
		this.x = x;
		this.y = y;
	...
	public double scale(int time)
	{
		return 1.5 + Math.sin(((double)time) / 5);
	}
}